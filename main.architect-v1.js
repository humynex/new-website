...
canvas.addEventListener('click', () => {
  if (hoveredPanel && hoveredPanel.userData.bodyMat && hoveredPanel.userData.bodyMat.opacity > 0.2) openModal(hoveredPanel.userData);
  platformPanels.forEach(({ panel, proj }) => { if (panel.userData._hov) openModal(proj); });
});
...
function updateUI(sp) {
  ...
  // Merge text (section 7 mid — DNA finale)
  showText(mergeText, s7>0.4&&s7<0.85);
  // Unified text (section 7 late — DNA fully revealed)
  showText(unifiedText, s7>0.7);
  // Services text — covers gap after hero AND portal void (0.22–0.64)
  showText(servicesText, sp>0.22&&sp<0.64);
  // AI world text — starts right where services ends
  showText(aiworldText, sp>0.62&&sp<0.87);
...
function animate(ts) {
  ...
  // ── AI WORLD ──
  const aiRawVis = clamp01((sp<0.78 ? (sp-0.62)/0.16 : (0.90-sp)/0.12));
  const aiVis = ease(aiRawVis);
  AI_WORLD.visible = aiVis > 0.005;
  if (aiVis > 0.005) aiWorld.show(); else aiWorld.hide();
