...
export default class DNAScene {
  ...
  _buildHelix() {
    ...
    const PAIRS   = 52;    // base pair count (more = denser, more realistic)
    const SEG     = 320;   // tube path segments
    const TUBE_R  = 0.21;  // tube cross-section radius
    const TUBE_PS = 36;    // tube radial segments (high = round & 3D)
    ...
    const nodeGeo = new THREE.SphereGeometry(NODE_R, 20, 14);
  ...
  _buildParticles() {
    ...
    this.particles = new THREE.Points(geo, this.mDust);
